...
2. **By network packet patterns.** 
  3. A MAC address, which are not included in EXCLUDE_MACS property of the config.txt file,  has asked for the MAC of the computer where is installed the BCAST_IDS, using ARP, IPv4 or IPv6 protocols.
  4. The computer where is installed the BCAST_IDs has generated ICMPv6 Parameter Problem message. 
