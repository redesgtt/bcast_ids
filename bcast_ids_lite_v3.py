...
mac_ipf = dict()
ts = 0
# Diccionario que almacena las direcciones MACs que han realizado una solicitud ARP, IPv4 o IPv6 a la sonda
macAccess_protocol = dict()
# Diccionario que almacena la cantidad de paquetes ICMPv6 Parameter Problem generados por la sonda hacia una determinada MAC destino
mac_icmpv6 = dict()
...
def mac_lines(pcap):
    ...
    for ts, payload in pcap:
        ...
        if mac_src in mac_line:
            ...
            # Comprobamos si la sonda ha generado paquetes de ICMPv6 Parameter Problem
            if mac_src == mac_sonda:
                if eth_type == '86dd':
                    icmpv6_param_problem(payload,mac_src,mac_dst,eth_type)

            # Contabilizamos la cantidad de paquetes generada por MAC
            # elif mac_src not in excluye_macs and mac_src in ipm_subred:
            elif mac_src not in excluye_macs:
                #print(mac_src)
...
"""Genera un diccionario de las direcciones MAC destino de los paquetes de ICMPv6 Parameter Problem ed la captura en curso"""
def icmpv6_param_problem(*args):
    payload= args[0]
    mac_src = args[1]
    mac_dst = args[2]
    eth_type = args[3]
    icmpv6_optype = binascii.hexlify(payload[54:55]).decode()
    icmpv6_opcode = binascii.hexlify(payload[55:56]).decode()
    if icmpv6_optype == '04' and icmpv6_opcode == '02':
        if mac_dst in mac_icmpv6:
            mac_icmpv6[mac_dst]+=1
        else:
            mac_icmpv6[mac_dst]=1
...
def run_IA():
    ...
    if os.path.isfile('./predict_iso_forest.py') and os.path.isfile('./model_iso_forest.bin'):
        ...
        # Tomamos la decision si aplicamos IA o se ha preguntado por la MAC origen de la sonda. Si la lista de macAccess_protocol esta vacia querra decir que no se ha preguntado por la sonda. A>
        if not macAccess_protocol and not mac_icmpv6:
            ...
            if macAccess_protocol:
                macs_atacando= list(macAccess_protocol.keys())
            if not macs_atacando and mac_icmpv6:
                macs_atacando= list(mac_icmpv6.keys())
...
def alerta_telegram(macs_atacando):
    ...
    for chat_id in chats_id:
        ...
        if macAccess_protocol:
            ...
            for mac_atacando in macs_atacando:
                message_to_telegram += f"The source MAC address '{''.join(map(str, mac_atacando))}' has generated {'/'.join(map(str,macAccess_protocol[''.join(map(str, mac_atacando))]))} packets towards the BCAST_IDS MAC address '{mac_sonda}'\n\n"
            if mac_icmpv6:
                for mac_atacando in macs_atacando:
                    if mac_icmpv6.get(mac_atacando)==1:
                        message_to_telegram += f"Moreover, the BCAST_IDS replied with {mac_icmpv6.get(mac_atacando)} ICMPv6 Parameter Problem message towards the MAC address '{''.join(map(str, mac_atacando))}'\n\n"
                    else:
                        message_to_telegram += f"Moreover, the BCAST_IDS replied with {mac_icmpv6.get(mac_atacando)} ICMPv6 Parameter Problem messages towards the MAC address '{''.join(map(str, mac_atacando))}'\n\n"
            message_to_telegram += f"The activity was: \n{cad}"

        elif mac_icmpv6:
            for mac_atacando in macs_atacando:
                if mac_icmpv6.get(mac_atacando)==1:
                    message_to_telegram += f"The BCAST_IDS MAC address '{mac_sonda}' has generated {mac_icmpv6.get(mac_atacando)} ICMPv6 Parameter Problem message towards the MAC address '{''.join(map(str, mac_atacando))}'\n\n"
                else:
                    message_to_telegram += f"The BCAST_IDS MAC address '{mac_sonda}' has generated {mac_icmpv6.get(mac_atacando)} ICMPv6 Parameter Problem messages towards the MAC address '{''.join(map(str, mac_atacando))}'\n\n"
            message_to_telegram += f"The activity was: \n{cad}"
    ...
    for mac_atacando in macs_atacando:
        if len(macs_atacando) == 1:
            if mac_atacando in macAccess_protocol:
                body =f"The source MAC address '{str(mac_atacando)}' sent {'/'.join(map(str,macAccess_protocol[''.join(map(str, mac_atacando))]))} packets towards the BCAST_IDS MAC address '{mac_sonda}' on {dia} at {hora}. "
                if mac_atacando in mac_icmpv6:
                    if mac_icmpv6.get(mac_atacando)==1:
                        body += f"Moreover, the BCAST_IDS replied with {mac_icmpv6.get(mac_atacando)} ICMPv6 Parameter Problem message towards the MAC address '{''.join(map(str, mac_atacando))}'. "
                    else:
                        body += f"Moreover, the BCAST_IDS replied with {mac_icmpv6.get(mac_atacando)} ICMPv6 Parameter Problem messages towards the MAC address '{''.join(map(str, mac_atacando))}'. "
            elif mac_atacando in mac_icmpv6:
                    if mac_icmpv6.get(mac_atacando)==1:
                        body += f"The BCAST_IDS, whose MAC address is {mac_sonda}, sent {mac_icmpv6.get(mac_atacando)} ICMPv6 Parameter Problem message towards the MAC address '{''.join(map(str, mac_atacando))}'. "
                    else:
                        body += f"The BCAST_IDS, whose MAC address is {mac_sonda}, sent {mac_icmpv6.get(mac_atacando)} ICMPv6 Parameter Problem messages towards the MAC address '{''.join(map(str, mac_atacando))}'. "
            else:
                AI_applied = True
                body = f"BCAST_IDS has detected that MAC address '{str(mac_atacando)}' had a suspicious behavior, using a Machine Learning algorithm, on {dia} at {hora}. "
        else:
            if mac_atacando in macAccess_protocol:
                body =f"The source MAC address '{str(mac_atacando)}' sent packets using the following protocols towards the BCAST_IDS MAC address destination '{mac_sonda}' on {dia} at {hora}. "
                ...
                AI_applied = True
                body = f"BCAST_IDS has detected that MACs '{', '.join(map(str,macs_atacando))}' had a suspicious behavior, using a Machine Learning algorithm, on {dia} at {hora}. "
        body += "See attached network capture (.cap file) for more information."
...
if __name__ == '__main__':
    try:
        ...
        if ipm:
            generate_file('./ipm.json', ipm, AGING4)
