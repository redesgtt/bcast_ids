...
def getValuesConfig(text):
    ...
    for line in lines:
        if text in line and not encontrado:
            encontrado = True
            if text == "NET" or text == "BANANA":
                values.append(line.rstrip().split("=")[1][1:-1])
            elif text == "EXCLUYE_MACS":
...
"""Creamos FLAG1 y FLAG2 para marcar en el dataset que haya ataques"""
def check_attack():
    global SALIDA
    if os.path.isfile('./flag1') or os.path.isfile('./flag2'):
        SALIDA = 1
        if os.path.isfile('./flag2'):
            os.system("rm ./flag2")
    return SALIDA
...
Formato: {mac : 'MACs, UCAST, MCAST, BCAST, ARPrq, ARPpb, ARPan, ARPgr, LIPF, IP_ICMP, IP_UDP, IP_TCP, IP_RESTO, IP6, RESTO, ARP_noIP, SALIDA'} """
def mac_lines(pcap):
    ...
    # SALIDA. Anexa a la ultima fila del dataset el valor 0 (si no hay ataque programado) o 1.
    for _mac, _values in mac_line.items():
        if _mac == mac_ataque_dataset:
            _values.append(SALIDA)
        else:
            _values.append(0)
...
if __name__ == '__main__':
    ...
    # Avisa si se ha detectado una MAC que no se encuentra en el JSON './tm-month.json'
    check_macs()
